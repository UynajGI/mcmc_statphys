...
class WangLandau():
    ...
    def _flat(self, array, epsilon=0.8):
        ...
        return min(
            nparray[nparray > 0]) > epsilon * np.mean(nparray[nparray > 0])
        ...
        with tqdm(total=total) as pbar:
            while self.logF > epsilon:
                if not self.elst:
                    ...
                    site = tuple(
                        np.random.randint(0, self.model.L,
                                          size=self.model.dim))
                    temp_model = copy.deepcopy(self.model)
                    self.model._change_delta_energy(site)
                    index_old = np.argmin(np.abs(self.elst -
                                                 temp_model.energy))
                    index_new = np.argmin(np.abs(self.elst -
                                                 self.model.energy))
                    ...
                    if abs(self.elst[index] -
                        ...
                        if self.elst[index] - self.model.energy > 0:
                            ...
                            self.elst = self.elst[:index + 1] + [
                                self.model.energy
                            ] + self.elst[index + 1:]
                            self.logG = self.logG[:index + 1] + [
                                self.logF
                            ] + self.logG[index + 1:]
                            self.hist = self.hist[:index + 1] + [
                                1
                            ] + self.hist[index + 1:]
                    else:
                        self.hist[index] += 1
                ...
                if count % 1000 == 0:
                    ...
                    pbar.set_description(
                        ...
                        .format(c=count // 1000,
                                ...
                                e=np.round(0.8 * np.mean(narray[narray > 0]),
                                           3)))
    ...
    def energy(self, T):
        elst = np.array(self.elst)
        logG = np.array(self.logG)
        return np.dot(elst, np.exp(logG - 1 / T * elst)) / np.sum(
            np.exp(logG - 1 / T * elst))

    def heat(self, T):
        elst = np.array(self.elst)
        logG = np.array(self.logG)
        return ((np.dot(elst**2, np.exp(logG - 1 / T * elst))) /
                np.sum(np.exp(logG - 1 / T * elst)) - self.energy(T)**2) / T**2
